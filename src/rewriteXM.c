...
void pcm8to16(const int8_t* in, int16_t* out, uint32_t sampleCount) {
    for (uint32_t i = 0; i < sampleCount; i++) {
        const int8_t inSample = in[i];
        int8_t mag = inSample;
        if (mag < 0) {
            mag = -mag; // Take the absolute value
        }

        // Convert to 16 bit magnitude
        int16_t outSample = (int16_t)(mag << 8) | mag;
        if (inSample < 0) {
            outSample = -outSample; // Match the input sign
        }
        out[i] = outSample;
    }
}
...
void pcm16to8(const int16_t* in, int8_t* out, uint32_t sampleCount) {
    for (uint32_t i = 0; i < sampleCount; i++) {
        const int16_t inSample = in[i];
        int16_t mag = inSample;
        if (mag < 0) {
            mag = -mag; // Take the absolute value
        }

        // Convert to 8 bit magnitude by grabbing the now-unsigned top byte
        int8_t outSample = mag >> 8;
        if (inSample < 0) {
            outSample = -outSample; // Match the input sign
        }
        out[i] = outSample;
...
bool writeOpusSampleXM(VirtualIO* io, const void* data, uint32_t size, bool is16Bit) {
    ...
    if (!is16Bit) {
        // Convert to 16-bit then convert to PCM in-place
        void* pcm8 = malloc(sampleCount);
        pcmDeltaDecode8(data, pcm8, sampleCount);
        pcm8to16(pcm8, pcmData, sampleCount);
        free(pcm8);

        // pcm8to16(data, pcmData, sampleCount);
        // pcmDeltaDecode16(pcmData, pcmData, sampleCount);
    } else {
        // Just convert to PCM
...
bool decodeOpusSampleXM(VirtualIO* io, const void* data, uint32_t size, bool is16Bit) {
    ...
    if (!is16Bit) {
        pcm16to8(buf, buf, sampleCount); // Convert to 8-bit in-place
        pcmDeltaEncode8(buf, buf, sampleCount);
        bufSize /= 2;
    } else {
